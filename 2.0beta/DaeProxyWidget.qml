...
PluginComponent {
    ...
    property string daeConfigPath: "/etc/dae/config.dae"
    ...
    function toggleSingleNode(nodeName) {
        ...
        root.activeNodes = an
        // Deactivate all groups when manually picking single node
        var ng = root.groups.slice()
    ...
    function writeDaeConfig() {
        var cfg = root.generateDaeConfig()
        configWriteProc.command = ["sh", "-c", "cat > /tmp/dae_config.dae << 'DAEEOF'\n" + cfg + "DAEEOF\npkexec cp /tmp/dae_config.dae " + root.daeConfigPath + " 2>/dev/null || sudo cp /tmp/dae_config.dae " + root.daeConfigPath]
    ...
    function loadDaeConfig() {
        root.nodePool = []
        root.activeNodes = []
        var ng = root.groups.slice()
        if (ng[0]) { ng[0].nodes = []; ng[0].isActive = false }
        root.groups = ng
        ToastService.showInfo("Loading from dae persist.d...")
        configLoadProc.running = false
        configLoadProc.command = ["sh", "-c", "cat /etc/dae/persist.d/*.sub 2>/dev/null"]
    ...
    Process {
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                var raw = text
                if (!raw.trim()) {
                    ToastService.showError("persist.d empty — need sudo access. Run: sudo chmod -R 644 /etc/dae/persist.d/")
                    return
                }
                var nodes = root.parseSubscriptionResponse(raw)
                if (nodes.length > 0) {
                    root.nodePool = nodes
                    ToastService.showInfo("Loaded " + nodes.length + " nodes from dae persist.d")
                } else {
                    ToastService.showError("No proxy links found in persist.d data (" + raw.length + " bytes)")
                }
            }
        }
    }
    Process {
        ...
        onExited: (exitCode, exitStatus) => {
            if (exitCode === 0) {
                reloadProc.command = ["dae", "reload", "-c", root.daeConfigPath]
                reloadProc.running = true
            } else {
                ToastService.showError("Config write failed — need sudo. Config saved to /tmp/dae_config.dae")
            }
        }
    }
    Process {
                ...
                Rectangle {
                    ...
                    Column {
                        ...
                        Row {
                            ...
                            StyledText {
                                text: "Groups: " + root.groups.length
                                font.pixelSize: Theme.fontSizeSmall
                                color: Theme.surfaceVariantText
                            }
                            StyledText {
                ...
                Rectangle {
                    ...
                    Column {
                        ...
                        Row {
                            ...
                            DankButton {
                                ...
                                onClicked: {
                                    ...
                                    ToastService.showInfo("Subscription added. Click [↻ Load & Update] to fetch nodes.")
                                }
                            }
                        }
                        Rectangle {
                        ...
                        StyledText {
                            ...
                            text: "No subscriptions. [+ Add] a URL, then [↻ Load & Update]."
                            font.pixelSize: Theme.fontSizeSmall
                            color: Theme.surfaceVariantText
                    ...
                    Flickable {
                        ...
                        Column {
                            ...
                            StyledText {
                                ...
                                text: "No nodes. Click [↻ Load] to import from wing.db."
                                font.pixelSize: Theme.fontSizeSmall
                                color: Theme.surfaceVariantText
                                horizontalAlignment: Text.AlignHCenter
                                width: parent.width
                    ...
                    Column {
                        ...
                        Item {
                            ...
                            StyledText {
                                ...
                                anchors.verticalCenter: parent.verticalCenter
                                text: {
                                    var cnt = (root.groups[0] && root.groups[0].nodes) ? root.groups[0].nodes.length : 0
                                    return "proxy (" + cnt + ")"
                                }
