...
function generateSubscriptionSection(subs) {
    ...
    var valid = false
    for (var i = 0; i < subs.length; i++) { if (subs[i] && subs[i].url && subs[i].url.indexOf("http") === 0) { valid = true; break } }
    if (!valid) return ""
    var s = "subscription {\n"
    for (var i = 0; i < subs.length; i++) {
        var sub = subs[i]
        var url = sub.url || ""
        if (url.indexOf("https-file://") !== 0 && url.indexOf("http") === 0)
            url = "https-file://" + url.substring(url.indexOf("://") + 3)
        if (url) s += "    " + (sub.tag || "sub_" + i) + ": '" + url + "'\n"
    }
    s += "}\n\n"
    return s
}
